...
const authCtrl = {
	register: async (req: Request, res: Response) => {
		try {
			const { name, account, password, result } = req.body;

			const user = await Users.findOne({ account });
			if (user) return res.json({ msg: "Account already exist" });

			const passwordHash = await bcrypt.hash(password, 12);

			const newPassHash = await bcrypt.hash(result + account, 12);

			const newUser = { name, account, password: passwordHash };

			if (!newUser) return res.json({ msg: "Invalid authentication." });
			...
			const new_hash = new RecoveryToken(newHash)
			await new_hash.save()
